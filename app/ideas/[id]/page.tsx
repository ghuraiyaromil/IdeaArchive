...
export default async function IdeaDetailPage({ params }: PageProps) {
  ...
  const { data: ideaData, error: ideaError } = await supabase
    .from("ideas")
    .select("*, founder:profiles!founder_id(id, username, bio, role_type, created_at)")
    .eq("id", params.id)
    .single();
  ...
  // Leaderboard scores (may be null if < 1 rating)
  ...
  // All ratings with rater profiles
  const { data: ratingsData } = await supabase
    .from("ratings")
    .select("*, rater:profiles!rater_id(id, username, bio, role_type, created_at)")
    .eq("idea_id", params.id)
    .order("created_at", { ascending: false });

  const ratings = (ratingsData ?? []) as RatingWithRater[];

  // Current user's own rating (if logged in)
  let myRating: Rating | null = null;
  if (user) {
    myRating = ratings.find((r) => r.rater_id === user.id) ?? null;
  }
  ...
  // Connection status (investor only)
    ...
    <div className="max-w-5xl mx-auto px-4 py-12">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          ...
          <section>
            <h2 className="text-sm font-mono uppercase tracking-widest text-white/40 mb-4">
              Community Reviews{" "}
              <span className="text-white/20">({ratings.length})</span>
            </h2>
        ...
        <div className="flex flex-col gap-5">
            ...
            <div className="rounded-xl border border-white/10 bg-[#111] p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-2xl font-mono font-bold text-white">
                  {scores.overall_composite_score.toFixed(1)}
                  <span className="text-white/30 text-base">/5</span>
                </span>
              </div>
              ...
              <p className="text-xs text-white/30 font-mono mt-3">
                {scores.rating_count}{" "}
                {scores.rating_count === 1 ? "rating" : "ratings"}
              </p>
            </div>
          ) : (
            <div className="rounded-xl border border-white/10 bg-[#111] p-5">
          ...
          {/* Connection button (investor, not their own idea) */}
          ...
          {/* Rating form (logged in, not owner) */}
          {user && !isOwner ? (
            <RatingFormWrapper
              ideaId={idea.id}
              userId={user.id}
...
function ReviewCard({ rating }: { rating: RatingWithRater }) {
  const composite =
    (rating.market_size_score + rating.feasibility_score + rating.clarity_score) /
    3;
