  ...
  return (
    <nav className="sticky top-0 z-50 border-b border-white/10 bg-[#0a0a0a]/80 backdrop-blur-md">
      <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
        ...
        <Link
          href="/"
          className="font-mono font-bold text-white text-sm tracking-tight"
        >
        ...
        <div className="sm:hidden border-t border-white/10 bg-[#0a0a0a] px-4 py-3 flex flex-col gap-3">
          ...
          {!loading && profile ? (
            <>
              ...
              <Link
                href={`/profile/${profile.id}`}
                className="text-sm text-white/60 hover:text-white font-mono"
                onClick={() => setMenuOpen(false)}
              >
                @{profile.username}
              </Link>
              <button
                ...
                className="text-left text-sm text-white/40 hover:text-white/70"
              >
                Sign out
              </button>
            </>
